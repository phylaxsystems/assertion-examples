...
import {OnlyAccreditedCanMintAssertion as KYCAssertion} from "../src/ass4-kyc-whitelist.a.sol";
...
contract KYCAssertionTest is Test, CredibleTest {
    ...
    function _createData() private view returns (bytes memory) {
        return abi.encodePacked(type(KYCAssertion).creationCode, abi.encode(address(registry)));
    }
