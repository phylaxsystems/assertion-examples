...
contract TestPositionSumAssertion is CredibleTest, Test {
    ...
    function test_assertionValidDeposit() public {
        ...
        string memory label = "Valid deposit";

        // Associate the assertion with the protocol
        cl.addAssertion(label, protocolAddress, type(PositionSumAssertion).creationCode, abi.encode(protocol));

        // Make a valid deposit
        vm.prank(user1);
        cl.validate(
            label, protocolAddress, 0, abi.encodePacked(protocol.deposit.selector, abi.encode(user1, depositAmount1))
        );
    }

    function test_assertionInvalidDeposit() public {
        address protocolAddress = address(protocol);
        string memory label = "Invalid deposit";
        ...
        cl.validate(label, protocolAddress, 0, abi.encodePacked(protocol.deposit.selector, abi.encode(user1, 42 ether)));
    }

    function test_assertionMultipleDeposits() public {
        address protocolAddress = address(protocol);
        string memory label = "Multiple deposits";
