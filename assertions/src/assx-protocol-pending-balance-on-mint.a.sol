...
import {Staking} from "src/assX-protocol-pending-balance-on-mint.sol";
...
contract StakingAssertions is Assertion, Staking, Test {
    function triggers() public view override {
        triggerRecorder.registerCallTrigger(
            this.assertTotalPendingOnMint.selector,
            Staking.mint.selector
        );
    }
    ...
    function assertTotalPendingOnMint() public {
        ...
        PhEvm.CallInputs[] memory mintCalls = ph.getCallInputs(
            address(staking),
            Staking.mint.selector
        );
        ...
        for (uint256 i = 0; i < mintCalls.length; i++) {
            (uint256 minToMint, ) = abi.decode(
                mintCalls[i].input,
                (uint256, uint256)
            );
            expectedPostTotalPending += minToMint;
            require(
                ctx.postTotalPending == ctx.preTotalPending + ctx.ethersToMint,
                "TotalPendingInvalid"
            );
        }
        ...
        require(
            staking.getPendingEthers() == expectedPostTotalPending,
            "ExpectedTotalPendingInvalid"
        );
    }
